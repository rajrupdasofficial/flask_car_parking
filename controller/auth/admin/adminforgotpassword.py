...
def admin_password_reset_logic():
    ...
    try:
        ...
        if email and not otp and not new_password:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT id,name,email FROM admin WHERE email = %s;", (email,))
            existing_user = cursor.fetchone()
            cursor.close()
            conn.close()

            if not existing_user:
                lo("error", "No admin found with this email.", "forgotpassword.py")
