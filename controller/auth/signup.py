...
def signup():
    ...
    if email and password:
        ...
        user_data = {
            "uuid": str(uuid.uuid4()),  # Generate a UUID for the new user
            "name": name,
            "email": email,
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO users (uuid, name, email, password, created_at, phone_number, profilepictures, userrole)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (user_data['uuid'], user_data['name'], user_data['email'], user_data['password'], user_data['created_at'], user_data['phone_number'], 'none', 'user'))
